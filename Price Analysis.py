...
st.markdown(' ')
st.markdown(' ')
getlocdf = pd.DataFrame(data=[gettownlist,getcountrylist,getlatlist,getlonlist,{'siz':np.random.randn(1000) * 100}]).T
getproplocdf = pd.concat([getdf['name'],getlocdf],ignore_index=True,axis=1)
getproplocdf.columns=['Name','Town','Country','lat','lon','siz']
st.map(getproplocdf,latitude='lat',longitude='lon',size='siz',zoom=1,use_container_width=True)